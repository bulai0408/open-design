...
function candidateBinaryNames(def: Pick<RuntimeAgentDef, 'bin' | 'fallbackBins'>): string[] {
  return [def.bin, ...(Array.isArray(def.fallbackBins) ? def.fallbackBins : [])].filter(
    (value): value is string => Boolean(value),
  );
}

function firstNonExecutableCandidatePath(
  def: Pick<RuntimeAgentDef, 'bin' | 'fallbackBins'>,
): string | null {
  for (const dir of agentSearchDirs()) {
    for (const bin of candidateBinaryNames(def)) {
...
export function buildExecutableDiagnostic(
  def: Pick<RuntimeAgentDef, 'id' | 'name' | 'bin' | 'fallbackBins'>,
  ...
  const nonExecutablePath = firstNonExecutableCandidatePath(def);
