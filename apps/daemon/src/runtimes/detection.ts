...
function unavailableAgent(
  ...
  overrides: Partial<
    Pick<
      DetectedAgent,
      'path' | 'version' | 'executableCandidates' | 'diagnostics'
    >
  > = {},
): DetectedAgent {
  return {
...
async function probe(
  ...
  if (!launch.selectedPath || !launch.launchPath) {
    ...
    return unavailableAgent(def, {
      diagnostics: [buildExecutableDiagnostic(def, configuredEnv)],
    });
  }
  ...
  // so a single agent's detection wall is max(help, models, auth) rather
  // than the sum. `--help` capabilities are cached on `agentCapabilities`
  // for buildArgs to consult.
  const [caps, modelResult, auth] = await Promise.all([
    probeCapabilities(def, probePath, probeEnv),
    fetchModels(def, probePath, probeEnv),
    probeAgentAuthStatus(def, probePath, probeEnv),
  ]);
  if (caps) {
