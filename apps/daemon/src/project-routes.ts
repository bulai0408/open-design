...
export interface RegisterProjectFileRoutesDeps extends RouteDeps<'db' | 'http' | 'paths' | 'uploads' | 'node' | 'projectStore' | 'projectFiles' | 'conversations' | 'documents' | 'artifacts'> {}
...
export function registerProjectFileRoutes(app: Express, ctx: RegisterProjectFileRoutesDeps) {
  ...
  const { listFiles, searchProjectFiles, readProjectFile, resolveProjectDir, resolveProjectFilePath, parseByteRange, createProjectFolder, moveProjectFile, renameProjectFile, deleteProjectFile, writeProjectFile, sanitizeName, ensureProject, listTabs, setTabs } = ctx.projectFiles;
